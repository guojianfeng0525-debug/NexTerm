...
CREATE TABLE IF NOT EXISTS net_links (id TEXT PRIMARY KEY, source_node_id TEXT NOT NULL, target_node_id TEXT NOT NULL, protocol TEXT NOT NULL DEFAULT 'tcp', port INTEGER, link_type TEXT NOT NULL DEFAULT 'unknown', status TEXT NOT NULL DEFAULT 'unknown', source TEXT NOT NULL DEFAULT 'manual', evidence TEXT NOT NULL DEFAULT '', description TEXT NOT NULL DEFAULT '', manual_label TEXT NOT NULL DEFAULT '', hidden INTEGER NOT NULL DEFAULT 0, first_seen_at INTEGER NOT NULL, last_confirmed_at INTEGER NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
...
INSERT OR IGNORE INTO net_nodes VALUES ('observed:10.10.1.30','observed:10.10.1.30','10.10.1.30','','10.10.1.30','general','','observed-server','','',0,NULL,NULL,NULL,'never',NULL,${topologyNow},${topologyNow});
...
INSERT OR IGNORE INTO net_links VALUES ('e2e-link-observed','e2e-node-a','observed:10.10.1.30','tcp',6379,'cache','observed','auto','ss ESTABLISHED 10.10.1.30:6379','','',0,${topologyNow},${topologyNow},${topologyNow},${topologyNow});
