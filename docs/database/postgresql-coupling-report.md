...
| Workspace UI/state | `src/components/toolbox/tool-postgres.tsx`, `src/components/toolbox/tool-sqlite.tsx` | Both hosts compose toolbar, Navigator placement, query workspace, editor/result layout, and status UI while owning provider runtime wiring. | Batch 11 may extract only demonstrated shared workspace composition. | PostgreSQL defaults, connection fields, runtime adapters, and provider services. | Keep both hosts and pass provider-owned data/callbacks into a UI-only shell. |
