...
| --- | --- | --- | --- | --- | --- |
| Toolbox entry | `src/App.tsx`, `src/components/toolbox/toolbox-nav.tsx`, `src/lib/toolbox/toolbox-types.ts` | PostgreSQL, SQLite, and MySQL have provider hosts and entries. | Shared provider-selection infrastructure and `DatabaseConnectionDialogShell` dialog composition are implemented. | Provider display label/icon, fields, sections, validation, and behavior. | Preserve provider hosts; avoid a generic runtime route. |
| Workspace UI/state | `src/components/toolbox/database-workspace-shell.tsx`, `src/components/toolbox/tool-postgres.tsx`, `src/components/toolbox/tool-sqlite.tsx`, `src/components/toolbox/tool-mysql.tsx` | All three hosts use the shared UI-only shell for toolbar host, Navigator placement, query-tab host, workspace region, and optional status region. | Implemented shared composition boundary. | PostgreSQL defaults, connection fields, runtime adapters, table paging, and provider services. | Keep provider hosts and pass provider-owned data/callbacks into the shell. |
| Profile types | `src/lib/database/profile-types.ts`, `src/lib/database/postgresql-profile-adapter.ts` | Shared `DatabaseConnectionProfile` owns identity/metadata/provider identity; PostgreSQL config owns connection fields. | Shared envelope and typed provider configuration are implemented and reused by SQLite and MySQL P0. | SSL fallback mode, SSH, read-only, credentials, and PostgreSQL driver settings. | Keep each provider's typed config and adapter without imposing host/port on the envelope. |
| Persistence | `src/lib/toolbox/postgres-storage.ts`, `src/lib/toolbox/db.ts`, `src-tauri/src/db.rs` | The production storage cache receives shared PostgreSQL profiles through the adapter. `postgres_connections` remains an intentionally flat PostgreSQL persistence DTO. | Shared domain/profile boundary is implemented; SQLite and MySQL P0 retain isolated persistence. | Existing table name, normalized columns, and encrypted field layout. | Keep the PostgreSQL schema frozen; no generic storage migration is implied by current three-provider adoption. |
| Global lifecycle | `src/lib/storage-init.ts`, `src/lib/config-export-import.ts`, `src/lib/reencrypt.ts` | Hydration, archive import/export, and re-encryption preserve existing persistence/ciphertext while adapting profiles at the domain boundary. | Shared profile envelope is used by current provider persistence and archives. | PostgreSQL encrypted-field list and archive DTO mapping. | Retain app-lock crypto and archive v1 format; SQLite and MySQL keep their existing isolated persistence adapters. |
...
| Tauri IPC/state | `src-tauri/src/postgres.rs`, `src-tauri/src/sqlite.rs`, `src-tauri/src/mysql.rs`, `src-tauri/src/lib.rs` | `PostgresState`/`postgres_*`, SQLite runtime/`sqlite_*`, and `MysqlState`/`mysql_*` are current provider-specific frontend contracts. | No current shared runtime caller is proven. | Driver clients, catalog SQL, SQL parser details, identifier quoting, local-file semantics, and MySQL protocol behavior. | Keep IPC and runtime provider-specific; reconsider only after evidence of meaningful duplicated orchestration. |
| Connection security | `src-tauri/src/postgres.rs`, `ToolPostgres` | PostgreSQL uses explicit first-use SSH host-key confirmation, persists the accepted fingerprint on its profile, then requires that pin for every tunnel; SSH auth, TLS roots/mTLS and timeouts remain provider details. | Shared SSH/TLS descriptors and small presentation primitives only when a real caller exists. | PostgreSQL `allow`/`prefer`/`verify-*` semantics and driver connector mapping. | Preserve fail-closed host-key pinning; providers translate common transport descriptors into driver options. |
...
## Feature Batch 9 - Connection Profile Platform Coupling

- `DatabaseConnectionProfile<TProviderId, TProviderConfig>` is the saved-profile domain contract. It has no PostgreSQL dependency and does not require network fields.
- `PostgreSQLConnectionConfig` owns all current PostgreSQL host/auth/read-only/TLS/SSH settings. `postgresql-profile-adapter.ts` maps it to and from the existing flat persistence DTO.
- `ToolPostgres`, the connection dialog, storage cache, Navigator projection, and archive path use shared PostgreSQL profiles in production. Navigator node IDs still receive the unchanged persisted profile ID.
- `postgres_connections`, app-lock AES-GCM encryption, the encrypted-column re-encryption list, and archive v1's flat `postgresConnections` payload remain unchanged intentionally. No schema or Rust migration was needed.

## Risks To Preserve During Migration

- Do not weaken PostgreSQL's fail-closed SSH host-key pinning, certificate validation, mTLS support, bounded timeouts, quoted identifiers, or primary-key update validation in `postgres.rs`.
...
- `postgres_*`, `sqlite_*`, and `mysql_*` IPC are provider contracts. A generic IPC/runtime boundary, `DatabaseState`, and unified runtime facade remain deferred despite three-provider frontend/domain reuse.
...
## Current Three-Provider Context

Feature Batches 12 and 13 added and validated experimental MySQL P0 alongside
PostgreSQL and SQLite. `ToolPostgres`, `ToolSqlite`, and `ToolMySql` reuse the
shared frontend/domain architecture and `DatabaseWorkspaceShell`, while retaining
provider-owned adapters and independent runtime stacks: PostgreSQL uses
`postgres_*`/`PostgresState`, SQLite uses `sqlite_*`/`rusqlite`, and MySQL uses
`mysql_*`/`MysqlState` with `mysql_async`. This evidence does not change the
PostgreSQL-specific coupling recorded above and does not justify generic runtime
or IPC architecture. PostgreSQL now consumes `DatabaseConnectionDialogShell` for
dialog composition while retaining provider-owned SSH, TLS, read-only,
validation, and provider behavior.
