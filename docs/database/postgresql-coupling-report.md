...
| --- | --- | --- | --- | --- | --- |
| Toolbox entry | `src/App.tsx`, `src/components/toolbox/toolbox-nav.tsx`, `src/lib/toolbox/toolbox-types.ts` | PostgreSQL and SQLite have provider hosts and entries. | Shared provider-selection infrastructure is implemented. | Provider display label/icon. | Preserve provider hosts; avoid a generic runtime route. |
| Workspace UI/state | `src/components/toolbox/database-workspace-shell.tsx`, `src/components/toolbox/tool-postgres.tsx`, `src/components/toolbox/tool-sqlite.tsx` | Both hosts use the shared UI-only shell for toolbar host, Navigator placement, query-tab host, workspace region, and optional status region. | Implemented shared composition boundary. | PostgreSQL defaults, connection fields, runtime adapters, table paging, and provider services. | Keep both hosts and pass provider-owned data/callbacks into the shell. |
| Profile types | `src/lib/database/profile-types.ts`, `src/lib/database/postgresql-profile-adapter.ts` | Shared `DatabaseConnectionProfile` owns identity/metadata/provider identity; PostgreSQL config owns connection fields. | Shared envelope and typed provider config are implemented. | SSL fallback mode, SSH, read-only, credentials, and PostgreSQL driver settings. | Additional providers add a typed config and adapter without adding host/port to the envelope. |
| Persistence | `src/lib/toolbox/postgres-storage.ts`, `src/lib/toolbox/db.ts`, `src-tauri/src/db.rs` | The production storage cache receives shared PostgreSQL profiles through the adapter. `postgres_connections` remains an intentionally flat PostgreSQL persistence DTO. | Shared domain/storage boundary is implemented. | Existing table name, normalized columns, and encrypted field layout. | Keep schema frozen until a storage migration has a concrete second-provider caller. |
| Global lifecycle | `src/lib/storage-init.ts`, `src/lib/config-export-import.ts`, `src/lib/reencrypt.ts` | Hydration, archive import/export, and re-encryption preserve existing persistence/ciphertext while adapting profiles at the domain boundary. | Shared profile envelope is used by storage and archives. | PostgreSQL encrypted-field list and archive DTO mapping. | Retain app-lock crypto and archive v1 format; future providers add their own persistence adapter. |
...
| Tauri IPC/state | `src-tauri/src/postgres.rs`, `src-tauri/src/sqlite.rs`, `src-tauri/src/lib.rs` | `PostgresState`/`postgres_*` and SQLite runtime state/`sqlite_*` are current provider-specific frontend contracts. | No current shared runtime caller is proven. | Driver clients, catalog SQL, SQL parser details, identifier quoting, and local-file semantics. | Keep IPC and runtime provider-specific; reconsider only after evidence of meaningful duplicated orchestration. |
...
- `postgres_*` and `sqlite_*` IPC are provider contracts. A generic IPC/runtime boundary is deferred until multiple real providers show meaningful duplicated orchestration.
