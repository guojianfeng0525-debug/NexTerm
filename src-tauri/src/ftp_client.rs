...
mod tests {
    ...
    async fn test_ftp_crud_cycle() {
        ...
        // 4b. Upload a file
        let tmp_upload = std::env::temp_dir().join("nexterm_e2e_upload.txt");
        let upload_content = b"Hello from NexTerm E2E test!\nLine 2\n";
        ...
        let tmp_download = std::env::temp_dir().join("nexterm_e2e_download.txt");
        ...
        assert!(
            !root_entries.iter().any(|e| e.name == "nexterm_e2e_test"),
