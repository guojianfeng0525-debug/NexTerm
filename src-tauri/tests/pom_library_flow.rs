...
fn library_class_decompile_resolves_from_library_jar() {
    ...
    let main_paths: Vec<&str> = all
        .iter()
        .filter(|r| r.library_id.is_empty())
        .map(|r| r.entry_path.as_str())
        .collect();
    assert!(
        !main_paths.iter().any(|p| p.contains("PageUtils")),
