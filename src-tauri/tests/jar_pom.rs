...
<project>
  ...
  <dependencies>
    ...
    let has_strutil = dep_idx
        .entries
        .iter()
        .any(|e| e.class_name == "cn.hutool.core.util.StrUtil");
...
"#)
    ...
    jar_db::upsert_library(
        ...
        &jar_db::JarLibrary {
            id: lib_id.into(),
            project_id: pid.into(),
            name: "hutool-all-5.8.25.jar".into(),
            group_id: "cn.hutool".into(),
            artifact_id: "hutool-all".into(),
            version: "5.8.25".into(),
            jar_path: dep_jar.display().to_string(),
            jar_hash: dep_idx.jar_hash.clone(),
            class_count: dep_idx.class_count as i64,
            editable: false,
        },
    )
    .unwrap();
    ...
    for e in dep_idx.entries.iter().filter(|e| {
        e.class_name == "cn.hutool.core.util.StrUtil"
            || e.class_name == "cn.hutool.core.util.StrUtil$Builder"
    }) {
        jar_db::upsert_class(
            &conn,
            &jar_db::JarClassRow {
    ...
    jar_db::upsert_symbol(
        &conn,
        &jar_db::JarSymbol {
            id: "sym1".into(),
            class_id: format!("{lib_id}:cn/hutool/core/util/StrUtil.class"),
            project_id: pid.into(),
            name: "isEmpty".into(),
            kind: "method".into(),
            line: 5,
            signature: "public static boolean isEmpty(CharSequence str)".into(),
        },
    )
    .unwrap();
    ...
    let strutil = found
        .iter()
        .find(|c| c.class_name == "cn.hutool.core.util.StrUtil")
        .expect("StrUtil indexed");
