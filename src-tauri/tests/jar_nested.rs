...
fn nested_archive_flow() {
    ...
    std::fs::write(
        &app_src,
        "public class Hello { public String hi() { return \"hi\"; } }\n",
    )
    .unwrap();
    ...
    let fat_jar = dir.join("fat.jar");
    {
        ...
        let entries = [app_out.join("Hello.class"), inner_jar.clone()];
        for e in &entries {
            cmd.arg("-C")
                .arg(e.parent().unwrap())
                .arg(e.file_name().unwrap());
        }
    ...
    assert!(
        nested.iter().any(|n| n == "BOOT-INF/lib/dep-lib.jar"),
    ...
    assert!(
        idx.entries.iter().any(|e| e.class_name == "com.dep.Dep"),
        "Dep class indexed"
    );
    ...
    jar_db::upsert_project(
        ...
        &jar_db::JarProject {
            id: pid.into(),
            name: "fat.jar".into(),
            jar_path: fat2.display().to_string(),
            jar_hash: "x".into(),
            size: 0,
            class_count: 0,
            resource_count: 0,
    ...
    jar_db::upsert_library(
        ...
        &jar_db::JarLibrary {
            id: lib_id.into(),
            project_id: pid.into(),
            name: "[nested] dep-lib.jar|BOOT-INF/lib/dep-lib.jar".into(),
            group_id: String::new(),
            artifact_id: "dep-lib".into(),
            version: String::new(),
            jar_path: extracted.display().to_string(),
    ...
    assert!(
        classes
            .iter()
            .any(|c| c.class_name == "com.dep.Dep" && c.library_id == lib_id),
        "nested class indexed under lib"
    );
