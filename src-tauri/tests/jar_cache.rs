...
fn container_tree_scopes_are_isolated() {
    ...
    {
        let e = "com/zhbr/transfer/VO.class";
    ...
    assert!(
        !main_paths.contains(&"com/zhbr/transfer/VO.class"),
        "library class leaked into main tree: {main_paths:?}"
    );
    ...
    assert!(
        !lib_a_paths.contains(&"com/app/Main.class"),
        "main class leaked into library tree: {lib_a_paths:?}"
    );
...
fn modified_source_is_persisted_unmodified_is_not() {
    ...
    assert!(
        c.modified_source.is_none(),
        "unmodified class must not persist source"
    );
    assert!(!c.modified);
    ...
    jar_db::upsert_class(
        &conn,
        &jar_db::JarClassRow {
            modified_source: Some(edited.into()),
            modified: true,
