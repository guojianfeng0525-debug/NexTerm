...
fn make_java_jar(dir: &Path, rel_src: &str, pkg_dotted: &str, name: &str, body: &str, cp: &[&Path]) -> std::path::PathBuf {
...
fn pom_library_click_and_service_method_location() {
    ...
    assert!(dep_idx.entries.iter().any(|e| e.class_name.ends_with("PageUtils")), "PageUtils indexed");
    assert!(dep_idx.entries.iter().any(|e| e.class_name.ends_with("UserService")), "UserService indexed");
    ...
    let lib = jar_db::get_library(&conn, pid, &lib_id).unwrap().expect("library");
    let page_utils_entry = dep_idx.entries.iter().find(|e| e.class_name.ends_with("PageUtils")).unwrap().entry_path.clone();
    ...
    let src = decompile::decompile_class(&cf, &jd, page_utils_entry.strip_suffix(".class").unwrap(), None).unwrap();
    ...
    eprintln!("CLASSES: {:?}", all.iter().map(|r| (&r.library_id, &r.class_name, &r.kind)).collect::<Vec<_>>());
    let dotted = "com.zhbr.app.UserService";
    let row = all
        .iter()
        .find(|c| c.class_name == dotted && c.kind == "class" && c.library_id.is_empty())
        .or_else(|| all.iter().find(|c| c.class_name == dotted && c.kind == "class"))
    ...
    let jar_path = if row.library_id.is_empty() {
        ...
        std::path::PathBuf::from(jar_db::get_library(&conn, pid, &row.library_id).unwrap().unwrap().jar_path)
    };
    ...
    let hit = jar::extract_methods(&svc_src).into_iter().find(|m| m.name == "getUserName");
...
fn main_jar_service_method_location() {
    ...
    assert!(std::process::Command::new(jdk.javac_path.as_deref().unwrap())
        .arg("-d").arg(&svc_out).arg(&svc_src).status().unwrap().success());
    ...
    assert!(std::process::Command::new(jdk.javac_path.as_deref().unwrap())
        .arg("-cp").arg(&svc_out).arg("-d").arg(&ctl_out).arg(&ctl_src).status().unwrap().success());
    // Main jar with both.
    let main_jar = dir.join("main.jar");
    assert!(std::process::Command::new("jar").arg("cf").arg(&main_jar).arg("-C").arg(&svc_out).arg(".").arg("-C").arg(&ctl_out).arg(".").status().unwrap().success());
    ...
    let b = pool.refs.iter().find(|r| r.name.as_deref() == Some("methodB")).expect("methodB ref");
    assert_eq!(b.internal_type_name, "com/zhbr/app/Service", "owner is the service interface");
    ...
    let ctl_res = decompile::decompile_class(&ctl_cf, &ctl_jd, "com/zhbr/app/Controller", None).unwrap();
    let pos_ref = ctl_res
        .refs
        .iter()
    ...
    assert!(pos_ref.offset > 0 && pos_ref.len == "methodB".len(), "offset={} len={}", pos_ref.offset, pos_ref.len);
    let src_text = &ctl_res.source[pos_ref.offset..pos_ref.offset + pos_ref.len];
    assert_eq!(src_text, "methodB", "position must point at the token: {src_text}");
    println!("POSITION-BOUND REF PASS (offset={}, len={})", pos_ref.offset, pos_ref.len);
    ...
    let hit = jar::extract_methods(&src).into_iter().find(|m| m.name == "methodB");
    assert!(hit.is_some(), "interface method line found in: {src}");
    println!("MAIN-JAR SERVICE METHOD LOCATION PASS (line {})", hit.unwrap().line);
    std::fs::remove_dir_all(&dir).ok();
}
