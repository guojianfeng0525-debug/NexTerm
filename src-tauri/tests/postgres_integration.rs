...
async fn connects_queries_and_reads_catalog() {
    ...
    let password =
        std::env::var("NEXTERM_TEST_POSTGRES_PASSWORD").expect("test password is required");
    let database =
        std::env::var("NEXTERM_TEST_POSTGRES_DATABASE").expect("test database is required");
    ...
    tokio::spawn(async move {
        connection
            .await
            .expect("PostgreSQL driver should stay healthy");
    });
    ...
    assert_eq!(
        row.try_get::<_, String>(0).expect("database value"),
        database
    );
