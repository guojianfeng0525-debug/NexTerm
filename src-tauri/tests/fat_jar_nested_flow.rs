...
fn fat_jar_nested_dependency_class_decompiles() {
    ...
    assert!(
        idx.entries
            .iter()
            .any(|e| e.entry_path == "BOOT-INF/classes/com/zhbr/app/App.class"),
    ...
    assert!(
        nested_entry.ends_with("common-util.jar"),
        "nested: {nested_entry}"
    );
    ...
    assert!(
        !r_entry.starts_with("BOOT-INF/"),
        "nested entry has NO BOOT-INF prefix: {r_entry}"
    );
    ...
    let lib = jar_db::get_library(&conn, pid, &r_row.library_id)
        .unwrap()
        .expect("nested lib");
    assert_ne!(
        lib.jar_path,
        fat.display().to_string(),
        "must resolve to the EXTRACTED nested jar"
    );
    ...
    let src = decompile::decompile_class(
        &cf,
        &jd,
        r_row.entry_path.strip_suffix(".class").unwrap(),
        None,
    )
    .unwrap();
    ...
    println!(
        "FAT-JAR NESTED DEP CLASS PASS (nested={nested_entry}, entry={})",
        r_row.entry_path
    );
...
fn known_names_index_covers_nested_library_types() {
    ...
    std::fs::write(
        main_src.join("com/zhbr/app/App.java"),
        "package com.zhbr.app; public class App {}",
    )
    .unwrap();
    ...
    for (ename, extracted, nested_idx) in &nested {
        ...
        jar_db::upsert_library(
            ...
            &jar_db::JarLibrary {
                id: lib_id.clone(),
                project_id: pid.into(),
                name: format!("[nested] {}", ename.rsplit('/').next().unwrap()),
                group_id: String::new(),
    ...
    assert!(
        names.contains("com.zhbr.pscs.common.util.mybatisplus.utils.R"),
        "nested dep class indexed (dotted)"
    );
    assert!(
        names.contains("com/zhbr/pscs/common/util/mybatisplus/utils/R"),
        "nested dep class indexed (slash internal form)"
    );
