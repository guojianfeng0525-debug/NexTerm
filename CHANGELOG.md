...
- **探测命令收敛为只读白名单（服务器负载重设计）**：一次手动探测在远端只执行 `ip -o addr`、`cat /proc/net/tcp`、单进程 `find /proc/[0-9]*/fd -lname 'socket:[*'`（BusyBox 回退 `ls -l`）与防火墙转储，全部毫秒级；彻底删除 2.18.0 逐 fd `readlink` 的 fork 风暴（繁忙主机一次探测数万次 exec 的 CPU 尖峰根因）、进程名采集与路由表段。守卫测试静态断言探测脚本永不包含任何写入 / 变更性命令（`rm`、重定向、包管理、`iptables -A/-F` 等）。
- **服务级拓扑边（A:p1 → B:p2）**：通过 fd 归属（连接 inode → 进程 → 该进程监听端口）推导两端均为监听端口的服务依赖边（如 `web-01:80 → db-01:3306`）；临时端口只作解析中间值，不落库、不上图。归属不可得（非 root 看不到他人进程、发起进程无监听）时降级为 `A → B:p2`，不编造端口。
- **防火墙转储按需执行**：`probe_network_topology` 新增 `includeFirewall` 参数，前端按 10 分钟 TTL 缓存跳过 `iptables-save` / `nft list ruleset` 重转储；跳过时已存储的防火墙数据保持原样，不会被误标为缺失。
- **拓扑图所见即所得**：删除 2.18.0 的分组隔离精确匹配与地址黑名单（`100.64.0.0/10`、`172.16.0.0/12` 等）两层隐形过滤——数据库里有什么就画什么；「全部连接」视图显示全部节点与连线（跨组连线、Tailscale / 企业内网连线永远可见），分组下拉降级为纯视图过滤器。
- **资源列进页缓存**：工具箱服务器列表最近一次采样持久化到本地，进入页面立即显示上次数据而非空白占位，新采样到达后原地刷新；轮询节奏与命令不变。

### 新增

- **节点身份 = IP（一个 IP 一个节点）**：自动发现的 observed 节点 id 改为规范化 IP 全局唯一，启动时自动迁移合并旧的分组作用域重复节点（连线重定向、位置与标签保留）；多网卡多 IP 仍是一个节点。同 IP 永远归并为同一资产。
- **连线可隐藏**：连线右键菜单新增「隐藏关系 / 取消隐藏」，连线编辑器新增隐藏开关；隐藏的连线淡显（0.32 透明度），开启「显示隐藏」后可查看与恢复，探测永不删除或隐藏任何用户数据。

### 修复

- **应用无法关闭（2.18.0 回归）**：关闭拦截在 `preventDefault()` 后依赖 `destroy()` 关窗，但 capabilities 未授予 `core:window:allow-destroy`，IPC 被权限层拒绝导致窗口永远留着。现已补授 `allow-destroy` / `allow-close`，并在 `destroy` 失败时回退到进程退出；关闭时的快照落盘加 3 秒超时，存储卡死不再把用户困在应用里。单测覆盖权限缺失回退与持久化挂起两条路径。
- **跨组连线消失**：连线要求两端节点都可见才绘制，而 2.18.0 将节点按分组精确过滤并允许 saved 节点分组随连接目录漂移（observed 节点只在重启时修复），导致移动连接目录后连线在会话内从两个分组视图同时消失。分组已不再参与节点身份与可见性判定，该问题从模型上消除。

## [2.18.0] - 2026-09-08

### 新增

- **工作空间改为关闭快照**：运行期间只维护最新内存快照，不再在每个标签状态变化后写入 SQLite；桌面窗口收到关闭请求时先等待一次原子快照落盘，再关闭窗口。正常关闭后重新打开恢复的即是关闭前最后状态，崩溃后也只会回到上一次正常关闭的工作空间。
...
- **Windows/Linux 窗口内菜单精简**：左上方只保留「服务器」和「终端」；应用、密码库、隧道、服务、记录本和网络拓扑菜单已从窗口工具条移除，工具箱导航仍可进入这些功能。「退出」菜单现在走统一关闭流程，确保先保存工作空间。
...
- **网络拓扑彻底移除主动连通性测试**：删除 `probe_tcp_ports` IPC 与界面入口，拓扑功能不会再从 A 服务器连接 B 服务器的任何 IP/端口，也不提供 `nc`、`/dev/tcp`、ping、DNS 或端口扫描路径；仅读取用户当前 SSH 会话所在服务器本机的 `/proc` socket 表。

### 修复

- **Linux 拓扑改为零依赖内核 Socket 采集**：优先读取 `/proc/net/tcp`、`/proc/net/tcp6`、`/proc/net/udp` 与 `/proc/net/udp6`，不再依赖 `ss`/`netstat`/`conntrack`，不安装工具，不截断 socket 行；同时保留 `ESTABLISHED`、`SYN_*`、`FIN_WAIT*`、`CLOSE_WAIT`、`TIME_WAIT` 等状态，短连接结束后的近期关系不会因为不再是 ESTABLISHED 而消失。
- **服务器与端口连线按双端被动证据归并**：IPv4-mapped IPv6 与 IPv4 地址会归一化，Docker/双栈场景下 A、B 两侧的正反向 socket 记录可确认为同一条关系；未知对端仍绘制为未探测服务器，不生成端口数据。
- **端口拓扑只显示真实监听端口**：客户端临时端口仅保留在内部五元组中用于方向和去重，不再出现在图、表格、详情或手工编辑器中；无法归属到本机监听进程的出站关系会绘制服务器级关系，不会伪造端口节点。手工端口连线只能选择已采集的真实监听端口，未采集对端只能连接到服务器节点。
- **防火墙状态与连通性解耦**：无活跃防火墙时端口详情显示“不适用”，活跃防火墙仅显示“策略元数据”；防火墙规则不再被用于推导拓扑边或端口可达性。
- **侧边网络面板布局优化**：状态与降级提示区域限制为可滚动且最多占面板高度 45%，五个标签平均分布并允许省略，正文保持 `min-height: 0`，修复中断/紧凑页面中端口、路由和防火墙内容被挤压显示不全的问题。

## [2.17.1] - 2026-09-07

### 修复

- **网络拓扑补全服务器与端口连线**：探测当前服务器时，`ss`/`netstat` 中已出现的未知对端现在会绘制为“观测到的服务器（未探测）”占位节点并生成服务器级连线；不会主动探测、扫描这些服务器，也不会伪造其端口数据。端口级拓扑保留入站/出站方向，能直接看到哪个服务器正在访问当前端口；用户后续显式探测该服务器时，占位节点会原位升级，已有关线和手工标注不丢失。
- **TCP 连通性改为从选中的服务器发起**：测试命令复用现有 SSH 会话（包括通过跳板机建立的会话），不再从本机探测；远端脚本优先使用 `nc`，没有 `nc` 或 `nc` 参数不兼容时回退到 `timeout + bash /dev/tcp`，再回退到 `bash + sleep + kill` 的原生超时方案，并区分可达、拒绝、超时、DNS 与执行错误。
- **网络拓扑全部弹窗布局修复**：统一审计 TCP 测试、节点编辑/删除、服务器连线编辑/删除、端口编辑、端口连线编辑/删除以及全局删除确认共 10 个弹窗；全部改为 Tauri 视口内居中、宽度按视口自适应、最大高度受限且内部滚动，不再出现上半部分越界、错误偏移或尺寸异常。
- **防火墙识别与端口判定适配更多后端**：firewalld/UFW 未激活但 iptables 规则已加载时会识别 iptables；规则采集优先 `iptables-save` 并保留 nat/mangle/raw 表上下文，同时继续采集 firewalld、UFW、nftables 与 PF，DNAT/REDIRECT 等转发动作按放行处理。
- **跳板机候选只允许直连服务器**：连接对话框和远程隧道不再把自身需要通过其它跳板机访问的服务器列为跳板机，避免生成不可达的级联跳板配置。
...
- **网络拓扑与网络诊断**：新增面向 Windows 桌面端的网络资产探测与拓扑管理能力，服务器端零安装。
  ...
  - **采集内容**：主机名、操作系统、主 IP、网络接口（IPv4/IPv6/MAC/MTU/状态）、路由表（含默认路由识别）、防火墙类型与状态（firewalld / UFW / iptables / nftables / PF）、防火墙规则、监听端口及对应进程与服务。
  - **零安装与优雅降级**：全部依赖系统自带命令（`ip` / `ss` / `iptables` / `nft` / `firewall-cmd` / `ufw` / `hostname` 等），服务器端无需安装任何 Agent 或额外软件；工具缺失或无 root 权限时按段降级并给出明确提示，不影响其余数据采集。
  - **TCP 连通性验证**：由 Windows 客户端主动发起 TCP 连接，与服务器监听状态交叉判定，区分「可达 / 被防火墙拦截 / 实际未监听 / 不可达 / 异常可连 / DNS 解析失败」六种状态。该验证同为手动触发，不随探测自动执行。
  - **增量持久化与人工数据保护**：新增 8 张 `net_*` 表。探测结果按自然键增量合并，**只更新自动采集字段**；人工维护字段（显示名、服务器类型、环境、备注、端口用途、服务名称、拓扑关系说明）永不被重新探测覆盖。本次未再出现的条目不删除，仅标记缺失时间并在界面置灰展示。
  - **全局拓扑持续合并**：依次探测多台服务器后，系统依据观察到的 ESTABLISHED 连接自动确认跨服务器关系；对端 IP 未匹配到已探测节点时一律丢弃。节点与连线支持查看、编辑、隐藏、删除与拖拽布局，自动推断（虚线）与人工维护（实线）在图例中区分。
  - **数据安全**：8 张表均不含密码、私钥、口令等任何认证字段，节点仅以连接 ID 引用已保存的连接；命令输出仅保留截断后的排障摘要，不落盘终端操作内容。
- **端口拓扑（二级下钻）**：在服务器拓扑上新增端口级链路能力，复用现有服务器资产、探测结果与持久化机制。
  - **服务器 → 端口 → 连接 → 对端端口**：选中服务器可查看端口列表，点击端口进入以 `IP:PORT/协议` 为中心的拓扑；入站 / 出站连接分开统计与筛选，可继续打开已持久化的对端端口，展开过程不触发任何网络探测。
  - **端口身份与状态分离**：端口按所属服务器 + 协议 + 监听地址 + 端口区分；界面同时展示监听状态、防火墙结论、客户端 TCP 可达性、实际连接、PID / 进程、服务 / 用途、关联数量与最近探测时间，避免将监听误判为可访问。
  - **未知端点安全处理**：`ss/netstat` 观察到的未探测 IP 仅保存为 `IP:port` 端点，不自动探测；用户之后主动探测该服务器时才与本机资产匹配关联。
  - **人工数据保护与手工关系**：新增 `net_port_links` 表，端口备注 / 标签 / 服务 / 用途及端口连接说明、标签、隐藏状态不会被重新探测覆盖；支持手工新增、编辑、删除入站或出站端口连接。
