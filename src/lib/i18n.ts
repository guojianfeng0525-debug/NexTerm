...
import { prefGet, prefSetRaw } from './preferences';

/** Keys used by the native macOS menu bar */
const NATIVE_MENU_KEYS = [
  'menuBar.servers', 'menuBar.terminal', 'menuBar.edit', 'menuBar.window',
  ...
  'toolbox.apps.title', 'toolbox.apps.open',
  'toolbox.vault.title', 'toolbox.vault.open',
  'toolbox.tunnels.title', 'toolbox.tunnels.open',
  'toolbox.services.title', 'toolbox.services.open',
  'toolbox.notes.title', 'toolbox.notes.open',
] as const;
...
const STORAGE_KEY = 'nexterm-language';

/**
 * Preference value meaning "follow the OS system locale", mirroring the
 * appearance theme picker's "auto" option. Stored in SQLite distinct
...
function readPreference(): string {
  return prefGet<string>(STORAGE_KEY, AUTO);
}
