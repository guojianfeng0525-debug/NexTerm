 ...
 *
 * Persisted in SQLite under the `preferences` table (key `nexterm-editor-config`).
...
const STORAGE_KEY = "nexterm-editor-config";

export function loadEditorConfig(): EditorConfig {
  try {
    const parsed = prefGet<Partial<EditorConfig> | null>(STORAGE_KEY, null);
...
export function saveEditorConfig(config: EditorConfig): void {
  prefSet(STORAGE_KEY, config);
}

// ---------- Event ----------

/** Dispatched on `window` whenever editor config is saved, so live editors can react. */
export const EDITOR_CONFIG_CHANGED_EVENT = "nexterm-editor-config-changed";
