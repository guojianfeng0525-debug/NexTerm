...
export interface TerminalTab {
  id: string;
  name: string;
  /** Tab type: 'terminal' for SSH PTY, 'file-browser' for SFTP/FTP, 'desktop' for RDP/VNC, 'editor' for remote file editing, 'tools' for the Toolbox workspace */
  tabType?: 'terminal' | 'file-browser' | 'desktop' | 'editor' | 'tools';
  protocol?: string;
  host?: string;
  username?: string;
  ...
  /** For tools tabs: the Toolbox view to show (apps/vault/tunnels/services/notes) */
  toolsTabView?: import('./toolbox/toolbox-types').ToolboxViewId;
}
...
export type TerminalGroupAction =
  ...
  | { type: 'UPDATE_TAB_TYPE'; tabId: string; tabType: TerminalTab['tabType'] }
  | { type: 'UPDATE_TOOLS_VIEW'; tabId: string; view: import('./toolbox/toolbox-types').ToolboxViewId }
