...
describe('vault-excel', () => {
  it('template round-trips through parse', () => {
    const bytes = buildVaultTemplate();
    const rows = parseVaultExcel(bytes);
    expect(rows.length).toBeGreaterThan(0);
    expect(rows[0].name).toContain('GitHub');
  });

  it('export → parse preserves records', () => {
    ...
    const parsed = parseVaultExcel(buildVaultExcel(rows));
  ...
  it('rejects a file without the "名称" header', () => {
    ...
    expect(() => parseVaultExcel(bytes)).toThrow(/名称/);
  });
