...
export function ConnectionManager({
  ...
  // Handle renaming folder. Delegates to the storage layer's `renameFolder`,
  // which atomically rewrites the folder, all subfolders AND every nested
  // connection's folder reference — so renaming never detaches servers from
  // their folder or loses any connection field.
  const handleRenameFolder = () => {
    ...
    try {
      ...
      if (ConnectionStorageManager.renameFolder(oldPath, newName)) {
        setConnections(loadConnections());
        ...
        toast.error(t('connectionManager.failedToRenameFolder'), {
          description: t('connectionManager.failedToRenameFolderDesc'),
        });
      }
    } catch (error) {
      toast.error(t('connectionManager.failedToRenameFolder'), {
