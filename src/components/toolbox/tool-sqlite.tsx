...
export function ToolSqlite() {
  ...
  return <DatabaseWorkspaceShell
    ...
    onCloseTab={(id) => setTabs((current) => current.filter((candidate) => candidate.id !== id))}
    ...
    workspace={tab && <section className="flex min-h-0 flex-1 flex-col"><div className="flex h-8 shrink-0 items-center gap-1 border-b px-2"><ToolButton icon={<Play />} label={t("toolbox.sqlite.run")} disabled={executeCommand.state !== "enabled" || running} onClick={() => void execute()} testId="sqlite-run" />{explainCommand.state !== "hidden" && null}{running && <Loader2 className="h-3.5 w-3.5 animate-spin" />}</div><div className="min-h-0 flex-1"><CodeEditor value={tab.sql} onChange={(sql) => patchTab(tab.id, { sql })} language="sql" queryContext={connected ? createSqliteQueryEditorContext({ connectionId: draft.id, lookup: async () => (await invoke<readonly { readonly name: string }[]>("sqlite_catalog_objects", { connectionId: draft.id })).map((item) => item.name) }) : undefined} className="h-full" /></div><DatabaseResultPane result={tab.result} height={260} paged={false} onPrevious={() => undefined} onNext={() => undefined} labels={{ result: t("toolbox.sqlite.result"), message: t("toolbox.sqlite.message"), ready: t("toolbox.sqlite.ready"), null: t("toolbox.sqlite.null"), previous: t("toolbox.sqlite.previous"), next: t("toolbox.sqlite.next"), rowsRange: (from, to) => t("toolbox.sqlite.rowsRange", { from, to }) }} /></section>}
  >
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="!inset-0 !m-auto !h-[min(520px,calc(100vh-32px))] !w-[560px] !max-w-[calc(100vw-32px)] !translate-x-0 !translate-y-0 gap-0 overflow-hidden rounded-md p-0" data-testid="sqlite-connection-dialog"><DialogHeader className="border-b px-4 py-3"><DialogTitle className="text-sm">{t("toolbox.sqlite.connectionSettings")}</DialogTitle></DialogHeader><div className="min-h-0 flex-1 overflow-auto p-4"><div className="grid gap-3"><Field label={t("toolbox.sqlite.provider")}><DatabaseProviderSelect value="sqlite" disabled={connections.some((item) => item.id === draft.id)} onValueChange={(providerId) => { if (providerId !== "sqlite") { setDialogOpen(false); window.dispatchEvent(new CustomEvent("nexterm:database-provider-selected", { detail: providerId })); } }} /></Field><Field label={t("toolbox.sqlite.name")}><Input value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} /></Field><Field label={t("toolbox.sqlite.databaseFile")}><div className="flex gap-2"><Input value={draft.providerConfig.filePath} onChange={(event) => setDraft((current) => ({ ...current, providerConfig: { ...current.providerConfig, filePath: event.target.value } }))} /><Button type="button" variant="outline" onClick={() => void chooseFile()}>{t("toolbox.sqlite.browse")}</Button></div></Field><div className="flex items-center gap-2"><Switch checked={draft.providerConfig.readOnly} onCheckedChange={(readOnly) => setDraft((current) => ({ ...current, providerConfig: { ...current.providerConfig, readOnly } }))} /><Label>{t("toolbox.sqlite.readOnly")}</Label></div></div></div><div className="flex justify-end gap-2 border-t px-4 py-3"><Button size="sm" type="button" variant="outline" className="rounded-sm" onClick={() => void save()}>{t("common.save")}</Button><Button size="sm" type="button" className="rounded-sm" onClick={() => void connect()} disabled={connecting}>{t("toolbox.sqlite.connect")}</Button></div></DialogContent></Dialog>
