...
import { resetConnectionsCache } from '../lib/connection-storage';
import { resetProfileCache } from '../lib/connection-profiles';
...
const LANGUAGE_KEY = 'nexterm-language';

/** Seed the stores with sample data so export has something to bundle. */
async function seedStores(): Promise<void> {
  ...
  prefSet(
    ...
    JSON.parse('{"theme":"dark","autoReconnect":true}'),
  );

  // Language
  prefSetRaw(LANGUAGE_KEY, 'zh-CN');
}
...
describe('exportAllConfig', () => {
  it('collects all config data and writes a JSON bundle', async () => {
    mockSave.mockResolvedValue('/tmp/nexterm-config-2026-06-28.json');
    ...
    expect(path).toBe('/tmp/nexterm-config-2026-06-28.json');
    ...
    // App settings
    expect(bundle.data.appSettings.theme).toBe('dark');
    expect(bundle.data.appSettings.autoReconnect).toBe(true);
  ...
  it('omits connections block when no connections exist', async () => {
    // Clear connections + folders from the in-memory cache.
    resetConnectionsCache();
...
describe('importAllConfig', () => {
  ...
  it('imports all categories in replace mode', async () => {
    ...
    // Verify the stores were actually updated
    ...
    expect(prefGetRaw(LANGUAGE_KEY)).toBe('en');
  });
  ...
  it('dispatches editor config changed event on import', async () => {
    ...
    window.addEventListener('nexterm-editor-config-changed', listener);

    const { importAllConfig } = await import('../lib/config-export-import');
    await importAllConfig();

    expect(listener).toHaveBeenCalledTimes(1);
    window.removeEventListener('nexterm-editor-config-changed', listener);
  });
...
describe('export/import round-trip', () => {
  it('exported config can be imported back with all data intact', async () => {
    ...
    expect(prefGetRaw(LANGUAGE_KEY)).toBe('zh-CN');
  });
});
