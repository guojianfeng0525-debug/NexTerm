...
// ── buildConnectionTree visibility: servers must always show ──

describe('buildConnectionTree server visibility', () => {
  it('shows servers inside a subfolder node', () => {
    const server = createConnection('DB-Server', 'All Connections/Work');

    const tree = ConnectionStorageManager.buildConnectionTree();
    const root = tree.find(n => n.name === 'All Connections')!;
    const work = root.children!.find(n => n.name === 'Work')!;
    expect(work.type).toBe('folder');
    const names = work.children!.filter(c => c.type === 'connection').map(c => c.name);
    expect(names).toContain('DB-Server');
    expect(server.folder).toBe('All Connections/Work');
  });

  it('shows connections whose folder was orphaned (points to a missing folder)', () => {
    // Simulate legacy/dirty data: connection references a folder that no
    // longer exists in the folders list.
    const orphan = ConnectionStorageManager.saveConnection({
      name: 'Orphan-Server',
      host: '10.0.0.1',
      port: 22,
      username: 'user',
      protocol: 'SSH',
      folder: 'All Connections/GoneFolder',
    });

    // Tree must still surface it (at root level) instead of hiding it.
    const tree = ConnectionStorageManager.buildConnectionTree();
    const rootNames = tree
      .filter(n => n.type === 'connection')
      .map(n => n.name);
    expect(rootNames).toContain('Orphan-Server');
    expect(orphan.folder).toBe('All Connections/GoneFolder');
  });

  it('shows connections with an empty/missing folder at root level', () => {
    ConnectionStorageManager.saveConnection({
      name: 'NoFolder-Server',
      host: '10.0.0.2',
      port: 22,
      username: 'user',
      protocol: 'SSH',
    ...
    const rootNames = tree
      .filter(n => n.type === 'connection')
      .map(n => n.name);
    expect(rootNames).toContain('NoFolder-Server');
  });
});
