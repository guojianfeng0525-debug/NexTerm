...
16. **Desktop protocol conventions** (`desktop_protocol.rs` + `desktop-viewer.tsx`): pointer `button_mask` uses the JavaScript `MouseEvent.buttons` bit layout (1 left, 2 right, 4 middle — NOT the VNC order) plus momentary wheel flags 0x08 (up) / 0x10 (down). Keyboard `key_code` is a JS `keyCode`, translated to PS/2 set-1 scancodes in Rust — letters/digits do NOT share ASCII values with scancodes (A=0x1E, '1'=0x02, '0'=0x0B); the mapping table is unit-tested against regressions. Binary desktop frames on the WebSocket use command byte `0x03` with big-endian fields, mirroring the `0x01` PTY framing.
