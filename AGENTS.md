...
| `russh` | SSH protocol (keys built-in since 0.62) |
...
12. **WKWebView WebGL 花屏 (garbled terminal canvas)**: On macOS, WKWebView may discard the drawing buffer — and sometimes silently kill the whole GL context without firing `webglcontextlost` — whenever a canvas stops being composited: hidden terminal tabs (`display:none` via `TerminalTabPortalHost`), an occluded/minimized window, App Nap suspension, or plain idle time. Because xterm only re-renders dirty rows, nothing repaints and the screen stays garbled (花屏) until new output arrives. Three defences in `pty-terminal.tsx` (keep all of them):
    - **Always construct the addon as `new WebglAddon(true)`** — `preserveDrawingBuffer: true` keeps the last good frame in the buffer so an evicted composite shows stale-but-valid pixels instead of garbage.
    - **On `visibilitychange` → visible and `window` `focus`, run `recoverFromOcclusion()`**: check `gl.isContextLost()` on the canvas (rebuild the WebglAddon if dead) then `term.refresh(0, rows - 1)` to force a full-row repaint.
    - **Run the same recovery when a tab becomes active** — in-app tab switches fire neither `visibilitychange` nor `focus`; a plain `refresh()` would draw into a dead context and the tab would come back garbled. The routine is shared through `recoverRendererRef`.

    Windows/Chromium adds two lifecycle rules for `@xterm/addon-webgl` 0.19:
    - **Never call `WebglAddon.dispose()` directly in terminal cleanup.** 0.19 detaches the canvas but keeps its WebGL2 context alive until GC; cumulative tab/split churn eventually crosses Chromium's per-document context cap and the browser evicts an oldest *live* terminal. Use `disposeWebglAddon()` in `src/lib/webgl-lifecycle.ts`, which also invokes `WEBGL_lose_context.loseContext()`. Remove that workaround when a stable addon release includes xterm.js#6069.
    - **Do not clear a shared texture atlas on terminal mount.** Terminals with equal render config share one atlas; 0.19's `clearTextureAtlas()` invalidates only the calling renderer. A newly mounted tab used to clear the atlas in its first appearance effect and leave surviving siblings with stale glyph coordinates. Only clear on actual appearance changes and use `clearSharedTextureAtlas()`, which refreshes every registered live terminal after clearing. Re-evaluate against xterm.js#6042/#6055 when upgrading past 0.19.

13. **IronRDP 0.17 + russh 0.63 coexistence via vendored picky/sspi patches**: The `ironrdp` suite is at 0.17 (`ironrdp-tokio` 0.10, `ironrdp-tls` 0.2 `rustls`, `ironrdp-cliprdr-native` 0.7) alongside `russh` 0.63. This combination is impossible on plain crates.io: `ironrdp-connector` 0.10 pins `picky =7.0.0-rc.25` and `sspi ^0.21`, and both pin crypto prereleases (`curve25519-dalek =5.0.0-rc.1`, `p256 =0.14.0-rc.14`) that can never satisfy russh's stable (`^5` / `^0.14`) requirements. Upstream already moved to stable RustCrypto, so `src-tauri/vendor/` carries two minimal manifest patches, wired through `[patch.crates-io]` in `src-tauri/Cargo.toml`: `picky` 7.0.0-rc.26 (stable crypto) with its `version` renamed to 7.0.0-rc.25 to satisfy connector's exact pin, and `sspi` 0.21.3 with its dead apple-target rc-pinned dependency blocks removed (its sources import none of them). The direct dependency `sspi = "=0.21.3"` forces the resolver to pick 0.21.3 so the patch always applies (without it the resolver may backtrack to 0.21.0 and report a phantom conflict). Re-evaluate on the next ironrdp release: once ironrdp-connector depends on picky ≥ rc.26 and sspi ≥ 0.21.4, both patches can be dropped. Bonus: ironrdp-session 0.11 natively handles xRDP's slow-path `ShareDataPdu::Update` first paint (the reason for the 0.13-era hand-written converter, now deleted).

14. **ironrdp-async `Framed` futures are still not provably `Send`** (also applies to 0.10): the `FramedRead` GAT (`type ReadFut<'read>`) lacks a `Send` bound in the trait declaration (upstream TODO), so any future holding `Framed` reads — including `connect_begin`/`connect_finalize` and `read_pdu` — fails `tokio::spawn`/Tauri command Send checks with "higher-ranked lifetime error / Send is not general enough", even though ironrdp-tokio's concrete impls box their futures as `Send`. The workaround in `rdp_client.rs` is `SendPduFramed`: a private framing wrapper over plain `tokio::io` extension traits (provably Send for concrete streams) that reimplements `read_pdu`/`read_by_hint`/`write_all`, plus manual driving of the connector `Sequence` (`step_sequence`, `perform_credssp`) and the Deactivation-Reactivation `ConnectionActivationSequence` (concrete type, not `&mut dyn Sequence` — passing `&mut dyn` across an await trips the same check via `&dyn PduHint`). Everything else (ActiveStageBuilder, cliprdr 0.7 backend, graceful_shutdown, ProcessorBuilder with `bulk_decompressor`) uses the official 0.17 APIs. If ironrdp-async ever adds `Send` bounds to the GATs, `SendPduFramed` can be replaced by `TokioFramed` throughout.

15. **RDP framebuffer rows are strided**: `DecodedImage::data_for_rect` returns a slice spanning `(height-1) * stride + width * bpp` bytes — it includes the gap between each row's dirty columns and the end of the row. The WebSocket frame protocol (and the JS parser in `src/lib/desktop-frame.ts`) requires tightly-packed `width × height × 4` bytes, so `compact_rect()` in `rdp_client.rs` must copy row-by-row. The JS parser deliberately rejects frames whose payload length doesn't match the declared rectangle.

16. **Desktop protocol conventions** (`desktop_protocol.rs` + `desktop-viewer.tsx`): pointer `button_mask` uses the JavaScript `MouseEvent.buttons` bit layout (1 left, 2 right, 4 middle — NOT the VNC order) plus momentary wheel flags 0x08 (up) / 0x10 (down). Keyboard `key_code` is a JS `keyCode`, translated to PS/2 set-1 scancodes in Rust (RDP) or X11 keysyms (VNC) — letters/digits do NOT share ASCII values with scancodes (A=0x1E, '1'=0x02, '0'=0x0B); the mapping tables are unit-tested against regressions. Binary desktop frames on the WebSocket use command byte `0x03` with big-endian fields, mirroring the `0x01` PTY framing.

17. **VNC client is vnc-rs 0.5, protocol never hand-written** (`vnc_client.rs`): the RFB wire protocol, ZRLE decoding, and VNC-Auth (DES challenge-response) all live in the `vnc-rs` crate (github.com/HsuJv, actively maintained, async tokio). Our module is glue only: JS-keyCode→X11-keysym translation (letters apply Shift/CapsLock XOR like gtk-vnc), JS-buttons→RFB button mask (JS 1/2/4 left/right/middle → RFB 1/2/4 left/middle/right), framebuffer composition (`Copy` regions materialized from the local buffer), and a refresh loop. Two hard constraints: (a) **never advertise Tight encoding** — servers then send JPEG rectangles the frontend can't render; advertise ZRLE+CopyRect+Raw only; (b) **never use `recv_event()`** — it holds the engine's tokio `Mutex` across a blocking channel wait and starves `input()`; poll with `poll_event()` (try_recv) on a 5ms tick and drive updates with `X11Event::Refresh` every 33ms (first tick `FullRefresh`). vnc-rs supports only None-auth and VNC-Auth — no TLS/ANONYMOUS/VEncrypt; servers requiring TLS are out of scope (documented limitation). Clipboard is RFB `ServerCutText`/`ClientCutText`, Latin-1 only.
