...
13. **IronRDP version pinning (RDP client)**: The `ironrdp` suite is pinned to 0.13 (`ironrdp-tokio` 0.7, `ironrdp-tls` 0.2 with the `rustls` feature) because 0.14+ requires crypto prereleases that conflict with russh 0.62 (`ecdsa`, `curve25519-dalek`, `block-padding`), and `sspi` 0.16.1 only compiles against picky ≤ 7.0.0-rc.17 (rsa 0.9 era). The picky pin lives in `Cargo.lock` (`cargo update picky --precise 7.0.0-rc.17` after any major lockfile regen). Do NOT bump these without re-verifying the russh combination. The pinning rationale is also documented in `src-tauri/Cargo.toml`.

14. **ironrdp-async 0.7 futures are not provably `Send`**: `FramedRead`'s GAT (`type ReadFut<'read>`) lacks a `Send` bound in the trait declaration, and ironrdp-async's own `AsyncNetworkClient` trait has no `Send` supertrait — so any future holding `ironrdp_async::Framed` reads or `connect_finalize` (whose signature mentions `&mut dyn AsyncNetworkClient`) fails `tokio::spawn`/Tauri command `Send` checks with "higher-ranked lifetime error / Send is not general enough". The workaround in `rdp_client.rs` is `SendPduFramed`: a private framing wrapper over plain `tokio::io` extension traits (provably Send for concrete streams) that reimplements `read_pdu`/`read_by_hint`/`write_all`, plus manual driving of the connector `Sequence` (`step_sequence`, `perform_credssp`) instead of `connect_begin`/`connect_finalize`. If ironrdp is ever upgraded, check whether upstream added `Send` bounds (newer `ironrdp-async` releases do) before keeping this workaround.
