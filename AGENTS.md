...
12. **WKWebView WebGL 花屏 (garbled terminal canvas)**: On macOS, WKWebView may discard the drawing buffer — and sometimes silently kill the whole GL context without firing `webglcontextlost` — whenever a canvas stops being composited: hidden terminal tabs (`display:none` via `TerminalTabPortalHost`), an occluded/minimized window, App Nap suspension, or plain idle time. Because xterm only re-renders dirty rows, nothing repaints and the screen stays garbled (花屏) until new output arrives. Three defences in `pty-terminal.tsx` (keep all of them):
    ...
    Windows/Chromium 额外要求遵守 `@xterm/addon-webgl` 0.19 的两条生命周期规则：
    - **终端清理中绝不能直接调用 `WebglAddon.dispose()`。** 0.19 只分离 canvas，WebGL2 context 会存活到 GC；标签/分栏长期开关会越过 Chromium 每文档 context 上限，浏览器随后驱逐最旧的存活终端。必须使用 `/src/lib/webgl-lifecycle.ts` 的 `disposeWebglAddon()`，它会同时调用 `WEBGL_lose_context.loseContext()`。稳定版 addon 包含 xterm.js#6069 后再移除该 workaround。
    - **终端挂载时不得清空共享 texture atlas。** 渲染配置相同的终端共享一个 atlas；0.19 的 `clearTextureAtlas()` 只会让调用方渲染器失效。新挂载标签曾在首次外观 effect 中清空 atlas，使存活兄弟终端保留过期 glyph 坐标。只在真实外观变化时清空，并使用 `clearSharedTextureAtlas()`；它会刷新所有已注册存活终端。升级超过 0.19 时按 xterm.js#6042/#6055 重新评估。
